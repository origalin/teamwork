...
public class FinanceDataServiceImpl  extends UnicastRemoteObject implements FinanceDataService{
	...
	public ArrayList<PayDocPO> getunchekedPayDocList() {
		...
		//String sql="SELECT 付款单号,日期,钱,付款人,付款账号,付款类型,备注,是否被审批 FROM PayDocPOList WHERE 是否被审批=no";
		...
		for(int i=0;i<payDocno.size();i++){
			PayDocPO temp=new PayDocPO(payDocno.get(i),date.get(i),money.get(i),payer.get(i),account.get(i),payType.get(i),back.get(i));
	...
	public ArrayList<GatheringDocPO> getunchekedGatheringDocList() {
		...
		for(int i=0;i<GatheringDocno.size();i++){
			GatheringDocPO temp=new GatheringDocPO(GatheringDocno.get(i),date.get(i),money.get(i),courier.get(i),sendDocnoList.get(i));
	...
	public void savePayDocPO(PayDocPO po) {
		...
		String sql="UPDATE PayDocPOList SET 是否被审批='true'where 付款单号="+PayDocID+";";
	...
	public void saveGatheringDocPO(GatheringDocPO po) {
		...
		String sql="UPDATE GatheringDocPOList SET 是否被审批='true'where 收款单号="+GatheringDocID+";";
		SQL.databaseUpdate(sql);
		SQL.closeDatabase();
	...
	public void modifyAccountPO(String oldAccountName, String newAccountName) {
		// TODO Auto-generated method stub
		String sql="UPDATE account SET 账户名称='"+newAccountName+"'WHERE 账户名称='"+oldAccountName+"';";
		SQL.databaseUpdate(sql);
		SQL.closeDatabase();
	}
	...
	public AccountPO checkAccountPO(String accountName) {
		...
		SQL.databaseQuery(sql);
		try{
			AccountPO temp=new AccountPO(SQL.rs.getString("账户名称"),SQL.rs.getDouble("账户余额"));
			return temp;
		}catch(SQLException e){
	...
	public void createPayDoc(String payDocID, double money, String account,
		...
		String sql="INSERT INTO PayDocPOList VALUES('"+payDocID+"','"+getCurrentDate().toString()+"',"+money+",'"+account+"','"+payType+"','"+back+"';";
		SQL.databaseUpdate(sql);
		SQL.closeDatabase();
	...
	public PayDocPO getPayDocPO(String PayDocID) {
		...
		String sql="SELECT 付款单号,日期,钱,付款人,付款账号,付款类型,备注 FROM PayDocPOList WHERE 付款单号='"+PayDocID+"';";
		SQL.databaseQuery(sql);
		try{
			String temp=SQL.rs.getString("付款类型");
			PayType payType;
			...
			PayDocPO PO=new PayDocPO(SQL.rs.getString("付款单号"),SQL.rs.getDate("日期"),SQL.rs.getDouble("钱"),SQL.rs.getString("付款人"),SQL.rs.getString("付款账号"),payType,SQL.rs.getString("备注"));
			return PO;
		}catch(SQLException e){
	...
	public void createGatheringDoc(String GatheringDocID, Double money,
		...
		String sql="INSERT INTO GatheringDocPOList VALUES('"+GatheringDocID+"','"+getCurrentDate().toString()+"',"+money+",'"+courier_name+"','"+items+"';";
		SQL.databaseUpdate(sql);
		SQL.closeDatabase();
	}
	...
	public GatheringDocPO getGatheringDocPO(String GatheringDocID) {
		String sql="SELECT 收款单号,日期,钱,快递员,寄件单编号列表 FROM GatheringDocPOList WHERE 收款单号='"+GatheringDocID+"';";
		SQL.databaseQuery(sql);
		try{
			String temp=SQL.rs.getString("寄件单编号列表");
			String[]split=temp.split(",");
			...
			GatheringDocPO PO=new GatheringDocPO(SQL.rs.getString("收款单号"),SQL.rs.getDate("日期"),SQL.rs.getDouble("钱"),SQL.rs.getString("快递员"),numberList);
			return PO;
		}catch(SQLException e){
			System.out.println("付款单预览出错");
			e.printStackTrace();
